...
function BlockchainComparisonRace() {
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      for (let ci = 0; ci < 3; ci++) {
        ...
        // Advance progress
        cs.progress += (1 / 60) / chain.blockTime * spd;
  ...
  const controlBar = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" };
  const btnBase = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" };
  const btnActive = { ...btnBase, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" };
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <div style={controlBar}>
        <button style={btnBase} onClick={() => { stateRef.current = null; setRestartCount(c => c + 1); }}>↻</button>
        <button style={paused ? btnActive : btnBase} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
        <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
          {[0.5, 1, 2].map(s => (
            <button key={s} style={speed === s ? btnActive : btnBase} onClick={() => setSpeed(s)}>{s}x</button>
          ))}
        </div>
      </div>
    </div>
...
function InteractiveHashChain() {
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      // Auto-add hash every ~12 frames (~200ms), scaled by speed
      const hashInterval = Math.max(1, Math.round(12 / speedRef.current));
      if (frameCount % hashInterval === 0) {
  ...
  const controlBar = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" };
  const cbtnBase = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" };
  const cbtnActive = { ...cbtnBase, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" };
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <div style={controlBar}>
        <button style={cbtnBase} onClick={() => { stateRef.current = { hashes: [], offset: 0, insertTx: false, verifying: false, verifyStart: 0, verifyDone: false, verifyFade: 0 }; setRestartCount(c => c + 1); }}>↻</button>
        <button style={paused ? cbtnActive : cbtnBase} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
        <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
          {[0.5, 1, 2].map(s => (
            <button key={s} style={speed === s ? cbtnActive : cbtnBase} onClick={() => setSpeed(s)}>{s}x</button>
          ))}
        </div>
      </div>
    </div>
...
function InnovationPipeline() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      if (!pausedRef.current) setCurrentStage(s => (s + 1) % 8);
    }, 2000 / speed);
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const controlBar = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const btnBase = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const btnActive = { ...btnBase, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={controlBar}>
          <button style={btnBase} onClick={() => setCurrentStage(0)}>↻</button>
          <button style={paused ? btnActive : btnBase} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? btnActive : btnBase} onClick={() => setSpeed(s)}>{s}x</button>
...
function PoHChainBuilder() {
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      // Add hash every ~6 frames (~100ms at 60fps), scaled by speed
      const hashInterval = Math.max(1, Math.round(6 / speedRef.current));
      if (frameCount % hashInterval === 0) {
        const doTx = st.insertTx;
        if (doTx) st.insertTx = false;
        addHash(doTx);
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <div style={controlBar}>
        <button style={cbtnBase} onClick={() => { stateRef.current = { hashes: [], particles: [], offset: 0, txCount: 0, hashCounter: 0, lastHashTime: 0, hashesPerSec: 0, recentHashes: 0, lastSecond: 0, insertTx: false }; setRestartCount(c => c + 1); }}>↻</button>
        <button style={paused ? cbtnActive : cbtnBase} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
        <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
          {[0.5, 1, 2].map(s => (
            <button key={s} style={speed === s ? cbtnActive : cbtnBase} onClick={() => setSpeed(s)}>{s}x</button>
...
function TowerBFTSimulator() {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => { setRound(0); setVotes(Array.from({ length: 12 }, () => ({ depth: 0 }))); setMalicious(new Set()); stateRef.current = { particles: [], finalized: false, finalizedBurst: false, ringPulses: Array.from({ length: 12 }, () => 0), blockPulse: 0 }; }}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
            ))}
          </div>
        </div>
      ); })()}
...
function ConsensusRace() {
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      if (!leftDone) {
        leftProgress = Math.min(1, leftProgress + leftRate * spd);
        if (leftProgress >= 1) leftDone = true;
      }
      if (!rightDone) {
        rightProgress = Math.min(1, rightProgress + rightRate * spd);
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => setNodeCount(c => { setPaused(false); return c; })}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
...
function GulfStreamForwarding() {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => setMode(m => m)}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
...
function TurbinePropagation() {
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      const spd = speedRef.current;
      if (st2.propagating) st2.frame += spd;
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={handlePropagate}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
...
function SealevelParallelViz() {
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      const schedInterval = Math.max(1, Math.round(30 / spd));
      if (st.frame % schedInterval === 0 && st.queue.length > 0) {
      ...
      for (let l = 0; l < numLanes; l++) {
        const tx = st.lanes[l];
        if (!tx) continue;
        tx.progress += spd / tx.duration;
        if (tx.glow > 0) tx.glow -= 0.02 * spd;
        if (tx.progress >= 1) {
          tx.state = "done";
          st.completed++;
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => { stateRef.current.initialized = false; stateRef.current.lanes = Array.from({ length: 16 }, () => null); stateRef.current.queue = []; for (let i = 0; i < 20; i++) stateRef.current.queue.push(generateTx(i)); stateRef.current.completed = 0; stateRef.current.tpsWindow = []; stateRef.current.nextId = 20; }}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
...
function CloudbreakIODemo() {
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      for (let i = 0; i < totalThreads; i++) {
        ...
        if (t.active) {
          t.dotProgress += t.dotSpeed * t.dotDir * spd;
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => { stateRef.current.threads = []; }}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
...
function TransactionJourney() {
  ...
  useEffect(() => { speedRef.current = speed; }, [speed]);
  useEffect(() => { animPausedRef.current = animPaused; }, [animPaused]);
  ...
  useEffect(() => {
    ...
    function loop() {
      if (!visible) { animRef.current = requestAnimationFrame(loop); return; }
      if (animPausedRef.current) { animRef.current = requestAnimationFrame(loop); return; }
      ctx.clearRect(0, 0, W, H);
      const now = performance.now();
      const spd = speedRef.current;
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => handleMode("auto")}>↻</button>
          <button style={animPaused ? ba : bb} onClick={() => setAnimPaused(p => !p)}>{animPaused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
            ))}
...
function ValidatorNetworkMap() {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => { stateRef.current = null; setAttackMode(false); }}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
...
function StakeWeightedQoS() {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => { setDots([]); statsRef.current = { prioritySent: 0, priorityDone: 0, regularSent: 0, regularDone: 0 }; idRef.current = 0; }}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
...
function OraclePriceFeed() {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => { stateRef.current.publishers = [ { basePrice: 150.00, weight: 1.0, price: 150.00 }, { basePrice: 150.10, weight: 1.0, price: 150.10 }, { basePrice: 149.80, weight: 0.5, price: 149.80 }, { basePrice: 150.20, weight: 0.5, price: 150.20 }, { basePrice: 149.95, weight: 0.5, price: 149.95 } ]; stateRef.current.dragging = -1; }}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
...
function AMMvsCLOB() {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => setTradeSize(10)}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
...
function JupiterRouteOptimizer() {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => { setRouted(false); setRouting(false); stateRef.current = { frame: 0, scanStart: 0, particles: [], routeResult: null }; }}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
...
function WalletTransactionFlow() {
  ...
  const wPausedRef = useRef(false);
  useEffect(() => { wPausedRef.current = wPaused; }, [wPaused]);
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={handleReplay}>↻</button>
          <button style={wPaused ? ba : bb} onClick={() => setWPaused(p => !p)}>{wPaused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
            ))}
...
function DePINCoverageMapper() {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={handleReset}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
...
function AgentDecisionLoop() {
  ...
  useEffect(() => {
    ...
    function loop(now) {
      if (!visible) { animRef.current = requestAnimationFrame(loop); return; }
      if (pausedRef.current) { lastTime = now; animRef.current = requestAnimationFrame(loop); return; }
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {(() => { const cb = { display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }; const bb = { fontSize: 11, padding: "4px 12px", borderRadius: 6, border: "1px solid #222228", background: "#141419", color: "#E8E6E1", cursor: "pointer", fontFamily: "'JetBrains Mono', monospace" }; const ba = { ...bb, background: "#14F195", color: "#0C0C0F", borderColor: "#14F195" }; return (
        <div style={cb}>
          <button style={bb} onClick={() => { stateRef.current = null; statsRef.current = { cycles: 0, opportunities: 0, profit: 0 }; setStats({ cycles: 0, opportunities: 0, profit: 0 }); }}>↻</button>
          <button style={paused ? ba : bb} onClick={() => setPaused(p => !p)}>{paused ? "▶" : "⏸"}</button>
          <div style={{ display: "flex", gap: 2, marginLeft: 8 }}>
            {[0.5, 1, 2].map(s => (
              <button key={s} style={speed === s ? ba : bb} onClick={() => setSpeed(s)}>{s}x</button>
